 ...
 *
 * $Log: radproto.cxx,v $
 * Revision 1.9  2004/08/11 03:05:15  zhanglu
 * GnuGK : RADIUS based call routing.
 *
...
void RadiusPDU::SetupMessageAuthenticator( 
	const PString& secret
	)
{
	unsigned char buf[MessageAuthenticatorLength];
	memset(buf, 0, MessageAuthenticatorLength);
	AppendAttr(RadiusAttr::MessageAuthenticator, buf, MessageAuthenticatorLength);

	/// generate 128-bit digest from shared secret and whole packet		
	hmac_md5(m_data, GetLength(), secret, secret.GetLength(), buf);

	RadiusAttr* const msgAuthAttr = const_cast<RadiusAttr*>(FindAttr(RadiusAttr::MessageAuthenticator));
	memcpy(msgAuthAttr->m_value, buf, MessageAuthenticatorLength);
}
	...
	)
{
	...
	for (unsigned i = 0; i < (m_roundRobinServers ? m_numRetries * numServers : numServers); i++)
	{
		...
		for (unsigned j = 0; j < (m_roundRobinServers ? 1 : m_numRetries); j++) {
			...
			if (RadiusPDU::AccountingRequest!=clonedRequestPDU->GetCode())	// zl added
				clonedRequestPDU->SetupMessageAuthenticator(*secret); // zl added
	      ...
	      unsigned char *digest)
{
        ...
        if (key_len > 64) {
                ...
                PMessageDigest5 tctx;
				PMessageDigest::Result result;
                
				tctx.Start();
				tctx.Process(key, key_len);
				tctx.CompleteDigest(result);
				memcpy(tk, result.GetPointer(), 16);
         ...
         * perform inner MD5
         */
        context.Start();	//(&context);                   /* init context for 1st pass */
		context.Process(k_ipad, 64);	//MD5Update(&context, k_ipad, 64);      /* start with inner pad */
        context.Process(text, text_len);//MD5Update(&context, text, text_len); /* then text of datagram */
        context.CompleteDigest(result);		//MD5Final(digest, &context);          /* finish up 1st pass */
		memcpy(digest, result.GetPointer(),result.GetSize());

        /*
         * perform outer MD5
         */
        context.Start();				// MD5Init(&context);                   /* init context for 2nd pass */
        context.Process(k_opad, 64);	//MD5Update(&context, k_opad, 64);     /* start with outer pad */
		context.Process(digest, 16);	//MD5Update(&context, digest, 16);     /* then results of 1st hash */
        context.CompleteDigest(result);		//MD5Final(digest, &context);          /* finish up 2nd pass */
		memcpy(digest, result.GetPointer(), result.GetSize());
}
//}} zl added
