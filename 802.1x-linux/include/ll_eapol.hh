...
namespace eap
{
	...
	//*** I/O
	//
	void receive(int ignore_errors = 0);
	void retransmit_last();
	void send(int ignore_errors = 0);
	...
	eapol::packet output()
	    {
		return eapol::packet(out_data + ll_hdr_len);
	    }
	...
	virtual void recv_frame(int ignore_errors) = 0;
	virtual void send_frame(ui8 *data, size_t len, int ignore_errors) = 0;
    };
}
